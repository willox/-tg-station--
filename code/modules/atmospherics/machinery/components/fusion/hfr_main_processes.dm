...
/obj/machinery/atmospherics/components/unary/hypertorus/core/process_atmos()
	...
	if(power_level == 6)
		critical_threshold_proximity = max(critical_threshold_proximity + max((round((internal_fusion.total_moles() * 9e5 + internal_fusion.return_temperature()) / 9e5, 1) - 2700) / 400, 0), 0)
	...
	if(internal_fusion.total_moles() > 0 && internal_fusion.return_temperature() < 5e5 && power_level <= 4)
		critical_threshold_proximity = max(critical_threshold_proximity + min(log(10, internal_fusion.return_temperature()) - 5, 0), 0)
...
//7 reworks
	...
	if(internal_fusion.return_temperature() <= FUSION_MAXIMUM_TEMPERATURE * temperature_modifier)
		...
		internal_fusion.adjust_temperature(-heat_limiter_modifier * 0.01 * delta_time)
	...
	if(waste_remove && power_level <= 5)
		...
		if(!ispath(filter_type))
			if(filter_type)
				...
				filtering = FALSE
		if(filtering && moderator_internal.has_gas(filter_type))
		...
		for(var/gas_id in selected_fuel.primary_products)
			if(internal_fusion.has_gas(gas_id))
		...
		if(internal_fusion.has_gas(/datum/gas/antinoblium))
